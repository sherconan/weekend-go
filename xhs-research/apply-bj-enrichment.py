"""
Apply bj-harvest/{slug}.json outputs to data-beijing-*.js files as
xhsQuote + xhsHeat fields after existing imageQuery (or last field).

Idempotent: skips dests that already have xhsQuote.

Usage:
  python3 apply-bj-enrichment.py [--dry-run] [--file data-beijing-500.js]
"""
...
def build_fields(data):
    ...
    heat = heat_tier(likes)
    return {
        'xhsQuote': xhs_quote,
        'xhsHeat': {'heat': heat, 'notes': notes, 'trending': ''},
    }

def is_bare_syntax(text):
    """Bare: id: 123 · quoted: "id": 123"""
...
def patch_file(rel_path):
    ...
    # Split into dest blocks. Each starts with "  {" at column 2 and ends with "  },"
    # Use a simple state machine: find all top-level {...}, block by block
    dest_blocks = []
    depth = 0
    start = None
    ...
    for (bstart, bend) in sorted(dest_blocks, key=lambda x: -x[0]):
        ...
        # Find last non-whitespace before bend-1
        j = bend - 2
        while j > bstart and out[j] in ' \n\t\r':
            j -= 1
        # Insert after j (after last real char of last field)
        # Need a comma after last existing field
        insert_str = ',\n' + addition + '\n  '
