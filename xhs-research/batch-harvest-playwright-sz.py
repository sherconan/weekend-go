"""SZ 159 dest XHS batch harvest — playwright headless + main Chrome cookies.

Clone of batch-harvest-playwright.py with SZ paths. Zero visible browser.
Idempotent (skip if {slug}.json exists). Single browser context for run.

Writes:
  - xhs-research/sz-harvest/{slug}.json
  - xhs-research/sz-harvest/_progress.txt
"""
...
QUEUE = f"{ROOT}/xhs-research/sz-queue.json"
...
def main():
    ...
    total = len(queue)
    log(f"=== SZ batch start · {total} dests ===")

    # Key insight: rednote-mcp's saved cookies (web_session + full login tokens) pass XHS anti-bot;
    # main-Chrome cookies via pycookiecheat triggered 安全验证 page.
    pw_cookies = json.load(open(os.path.expanduser('~/.mcp/rednote/cookies.json')))
    log(f"cookies injected: {len(pw_cookies)} (from rednote-mcp session)")
